    ...
    for DeserializeVirtualPackagePlugins
{
        ...
        D: Deserializer<'de>,
    {
        ...
        impl<'de> serde::de::Visitor<'de> for Entries {
            type Value = Option<Vec<(String, MaybeArray)>>;
            ...
            fn visit_map<A: serde::de::MapAccess<'de>>(
                ...
                while let Some(entry) = map.next_entry::<String, MaybeArray>()? {
                    entries.push(entry);
                }
                Ok(Some(entries))
            }
...
impl<'de> Deserialize<'de> for MaybeArray {
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Ok(match Value::deserialize(deserializer)? {
            Value::Array(names) => MaybeArray::Array(names),
            Value::Other(_) => MaybeArray::NotAnArray,
        })
    }
}
...
impl<'de> Deserialize<'de> for MaybeName {
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Ok(match Element::deserialize(deserializer)? {
            Element::Name(name) => MaybeName::Name(name),
            Element::Other(_) => MaybeName::NotAName,
        })
    }
}
