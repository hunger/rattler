...
/// Which channel speaks for a virtual package two channels both register is
/// decided by the CEP-42 order the channels resolve to, and the fixture pair
/// says so without the user listing either channel's base.
///
/// `virtual-package-plugins-derived` declares `virtual-package-plugins-base` as
/// its base, which outranks it, and both register a plugin for `__cuda`.
#[tokio::test]
...
/// A plugin's dependencies are resolved from the registering channel *and the
/// channels its CEP-42 relations reach*, which is what lets a channel deriving
/// from another ship a detector built against it.
///
/// `vendor-cuda-detect` is served by `virtual-package-plugins-derived` and
/// depends on `vendor-lib`, which only its base channel serves. Nothing else in
/// the test names that base.
#[tokio::test]
