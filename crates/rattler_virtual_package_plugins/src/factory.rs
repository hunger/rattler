...
mod tests {
    ...
    /// What this client detected is recorded as this client's own, which a
    /// lockfile or a diagnostic downstream has no other way to tell from a
    /// channel's answer.
    #[tokio::test]
    ...
    /// A broken plugin and a machine without the hardware are indistinguishable
    /// from here, and neither is worth refusing to solve over: the name is
    /// absent, and the solver says what that costs in its own words.
    #[tokio::test]
    ...
    /// An unusable override is the user's own instruction, not a channel's
    /// mishap: carrying on with a detected value would look like it took effect.
    #[tokio::test]
