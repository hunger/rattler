...
mod tests {
    ...
    fn a_report_with_no_verdicts_parses_and_carries_no_policy() {
        let report = parse_report(r#"{"version": 1}"#).unwrap();
    ...
    /// A report that does not say which protocol it speaks cannot be read as
    /// this one: the client would be guessing at what the keys mean.
    #[test]
    ...
    /// A newer revision needs a newer client. Told apart from a broken plugin,
    ...
    fn a_plugin_that_declares_no_policy_still_gets_an_expiry() {
        ...
        let watching =
            parse_report(r#"{"version": 1, "cache": {"watch_paths": ["/dev/null"]}}"#).unwrap();
        assert_eq!(
    ...
    fn a_plugin_cannot_ask_to_be_cached_forever() {
        let forever =
            parse_report(r#"{"version": 1, "cache": {"ttl_seconds": 999999999}}"#).unwrap();
        assert_eq!(
    ...
    fn a_ttl_within_the_maximum_is_honoured() {
        let hour = parse_report(r#"{"version": 1, "cache": {"ttl_seconds": 3600}}"#).unwrap();
        assert_eq!(hour.cache.unwrap().effective_ttl_seconds(), 3600);

        // Zero is a plugin saying "do not reuse this", and is not overridden
        // into the default.
        let never = parse_report(r#"{"version": 1, "cache": {"ttl_seconds": 0}}"#).unwrap();
