...
mod tests {
    ...
    /// What an `activate.d` script exports has to reach the plugin, which is the
    /// whole reason for running activation at all.
    #[tokio::test]
    ...
    /// Activation puts the prefix in front of the caller's `PATH`, so a plugin
    /// calling a helper finds its own.
    #[tokio::test]
    ...
    /// A failing activation script is a broken environment, not something to
    /// run the plugin without: what it reported would depend on whether the
    ...
    /// Activation is inside the run's budget, so an environment that cannot be
    /// prepared in time fails the same way a plugin that cannot run in time
    /// does -- and says which of the two it was.
    #[tokio::test]
    async fn activation_is_bounded_by_the_run_deadline() {
        let prefix = tempfile::tempdir().unwrap();
        write_activation_script(prefix.path(), "sleep 5\n", "ping -n 6 127.0.0.1 >nul\r\n");
