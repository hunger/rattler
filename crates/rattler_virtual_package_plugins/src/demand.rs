...
mod tests {
    ...
    /// The forms a dependency on a virtual package actually takes in repodata.
    #[test]
    ...
    /// Ordinary packages are the overwhelming majority of what gets scanned and
    /// must not be mistaken for virtual ones.
    #[test]
    ...
    /// A plugin only runs if something mentions one of its names, so the set
    /// being empty is the case that saves the work.
    #[test]
