...
///
/// A registration for more names than [`MAX_VIRTUAL_PACKAGES_PER_PLUGIN`] is
/// dropped whole rather than trimmed: which sixteen of them a channel meant is
/// not something a client can pick, and running a plugin against a set its
/// author never wrote would fail the contract check anyway.
fn fold_subdirs(
    registrations: impl IntoIterator<Item = SubdirVirtualPackagePlugins>,
    ...
    for (channel, plugins) in &mut channels {
        plugins.retain(|plugin, declared| {
            let within = declared.len() <= MAX_VIRTUAL_PACKAGES_PER_PLUGIN;
            if !within {
                tracing::warn!(
                    "ignoring the registration of '{}' by '{channel}': it names {} virtual \
                     packages, and no plugin may register for more than \
...
mod tests {
    ...
    /// One name, one answer. The channel priority the registrations arrive in
    /// is what picks it, and the loser is not run.
    #[test]
    ...
    /// Two channels that never contest a name both answer, whatever their
    /// order: shadowing is per name, not per channel.
    #[test]
    ...
    /// A plugin can lose one name to a higher-priority channel and keep
    /// another. It still runs, and is still held to everything its channel
    /// registered it for.
    #[test]
    ...
    /// something else is dropped rather than carried into a solve, where it
    /// would fail later as an unusable package spec.
    #[test]
    ...
    /// A registration is what bounds how much a plugin gets to say, so how far
    /// it reaches cannot be the channel's to choose.
    #[test]
    fn a_registration_for_too_many_names_is_dropped() {
        ...
        let ran: Vec<_> = resolved
            .plugins
            .iter()
            .map(|plugin| plugin.plugin.as_source())
            .collect();
        assert_eq!(
            ran,
            ["modest-detect"],
            "the registration at the cap survives, the one past it does not"
        );
        assert!(
            resolved.shadowed.is_empty(),
            "a dropped registration is not a shadowed one"
        );
    }

    /// A channel repeats a registration in every subdir, so what the cap
    /// applies to is their union rather than what any one subdir said.
    #[test]
