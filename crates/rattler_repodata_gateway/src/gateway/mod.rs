...
impl Gateway {
    ...
    /// [`ChannelRelationsMode::Warn`]: what they cost is reported in
    /// [`ResolvedChannels::warnings`].
    #[cfg(feature = "experimental-virtual-package-plugins")]
...
mod test {
    ...
    /// Both entry points take their order from the same channel ranks, so a
    /// resolution must list channels in the order a query over the same
    /// channels puts its buckets in. Were they to drift apart, a caller using a
    /// resolution as its channel priority would resolve packages from a channel
    /// the gateway itself would not have picked.
    #[cfg(feature = "experimental-virtual-package-plugins")]
    #[tokio::test]
    ...
    /// The channel names of a resolution, in the order it puts them.
    #[cfg(feature = "experimental-virtual-package-plugins")]
