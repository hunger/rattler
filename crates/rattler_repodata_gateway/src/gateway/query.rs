    ...
    Visit: FnMut(&ChannelUrl, Platform, &Subdir),
{
    ...
    // the caller gave, which is still the answer to the question. It comes back
    // in the same order `Gateway::query` sorts its buckets by, so a caller handed
    // this order resolves packages from the channel the gateway would pick.
    // Finalizing also reports the depth and cycle problems met on the way, so it
    // runs whether or not anything reads the order it produces.
