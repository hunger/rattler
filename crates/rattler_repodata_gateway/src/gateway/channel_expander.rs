...
impl ChannelExpander {
    ...
    ///
    /// This is the single definition of channel priority order. Everything that
    /// needs channels in priority order derives it from here — `Gateway::query`
    /// sorts its subdir handles by it and `walk_channels` orders its result by
    /// it — so the callers cannot drift apart.
    ///
...
///
/// The only way anything outside this module obtains channels in priority
/// order. The raw topological order stays private to the module, so a caller
/// cannot reach past the anchoring rule and end up with an order the gateway
/// does not actually apply — which is how the two orders drifted apart before.
